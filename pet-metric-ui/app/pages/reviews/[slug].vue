...
const listingSpecs = computed<SpecRow[]>(() => [
  ['Price', product.value.priceDisplay || 'Check current listing'],
...
const reviewSections = computed(() => [
  ...
  ...(userReview.value?.summary ? [{ title: 'Owner feedback themes', body: `${userReview.value.summary} Positive themes: ${(userReview.value.positives || []).join('; ') || 'Not captured'}. Concerns: ${(userReview.value.concerns || []).join('; ') || 'Not captured'}.` }] : []),
...
const evidenceStatus = computed(() => product.value.amazonVerified
  ? `Amazon.com listing verified${product.value.commerceCheckedAt ? ` on ${formatReviewDate(product.value.commerceCheckedAt, '')}` : ''}; editorial testing status is ${review.value.sourceCount ? 'documented' : 'not independently verified'}.`
