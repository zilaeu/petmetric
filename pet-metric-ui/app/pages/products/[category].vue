...
<template>
  <div>
    ...
    <section class="category-criteria" :class="{ 'category-criteria--litter': isLitterCategory }"><div class="container"><div v-for="(criterion, index) in effectiveMeta.criteria" :key="criterion[0] || criterion.title" class="category-criterion"><span>0{{ index + 1 }}</span><strong><i v-if="criterionIcons[index]" aria-hidden="true"><AppIcon :name="criterionIcons[index] as any" :size="16" /></i>{{ criterion[0] || criterion.title }}</strong><p>{{ criterion[1] || criterion.description }}</p></div></div></section>
