...
const filtered = computed(() => comparisons.value.filter(item => activeCategory.value === 'all' || item.categorySlug === activeCategory.value))
...
const resetFilters = () => { activeCategory.value = 'all' }
  ...
  <div>
    ...
    <section class="home-section"><div class="container">
      <div class="comparison-library-filters">
        <div class="filter-row"><span>Product type:</span><button :class="{active: activeCategory === 'all'}" :aria-pressed="activeCategory === 'all'" @click="activeCategory='all'">All categories</button><button v-for="item in categoryOptions" :key="item.slug" :class="{active: activeCategory === item.slug}" :aria-pressed="activeCategory === item.slug" @click="activeCategory=item.slug">{{ item.label }}</button><small>{{ filtered.length }} comparison{{ filtered.length === 1 ? '' : 's' }}</small></div>
      </div>
      <div v-if="filtered.length" class="comparison-design-grid"><button v-for="item in paged" :key="item.href" class="comparison-design-card" type="button" @click="openComparison(item)"><div class="comparison-card-header"><span>{{ item.category }} · {{ item.comparisonType || 'Direct alternative' }}</span><span>Checked {{ item.checkedAt || item.updated }}</span></div><div class="comparison-card-body"><div class="versus"><strong>{{ item.a }}</strong><span>vs</span><strong>{{ item.b }}</strong></div><p>{{ item.verdict }}</p><div class="comparison-best-list"><div v-for="best in item.bestFor" :key="best.pick"><b :class="{ neutral: best.pick === 'No recommendation yet' }">{{ best.pick }}</b><span>{{ best.who }}</span></div></div><div class="comparison-card-bottom"><span>{{ item.evidenceConfidence || 'Limited' }} evidence</span><strong>Quick comparison <AppIcon name="arrow" :size="13" /></strong></div></div></button></div>
      ...
      <div v-if="!filtered.length" class="empty-state comparison-filter-empty"><p>No comparison matches this product type yet.</p><button class="btn btn--light" type="button" @click="resetFilters">Show all comparisons</button></div>
    </div></section>
