  ...
  <div>
    ...
    <section class="home-section"><div class="container">
      <div class="comparison-library-filters">
        <div class="comparison-filter-controls"><label><span>Product type</span><select v-model="activeCategory"><option value="all">All categories</option><option v-for="item in categoryOptions" :key="item.slug" :value="item.slug">{{ item.label }}</option></select></label><label class="comparison-name-filter"><span>Product name</span><input v-model="productQuery" type="search" placeholder="Search either product…" /></label><small>{{ filtered.length }} comparison{{ filtered.length === 1 ? '' : 's' }}</small></div>
      </div>
